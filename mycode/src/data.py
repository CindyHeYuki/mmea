...
class EADataset(torch.utils.data.Dataset):
    def __init__(self, data,difficulty_dict=None):
        self.data = data
...
class Collator_base(object):
    ...
    def __call__(self, batch):
        # pdb.set_trace()
        # 检查批次数据类型
        if len(batch[0]) == 3:  # 包含难度的三元组
            ent1 = [item[0] for item in batch]
            ent2 = [item[1] for item in batch]
            difficulties = [item[2] for item in batch]
            return {
                'ent1': np.array(ent1),
                'ent2': np.array(ent2),
...
def load_eva_data(logger, args):
    ...
    #New1
    # 计算实体度数（用于结构稀疏度）
    ent_degree = np.zeros(ENT_NUM, dtype=np.int32)
    ...
    # 添加投影层
    img_dim = args.img_dim if hasattr(args, 'img_dim') else 300
    rel_dim = args.attr_dim if hasattr(args, 'attr_dim') else 300
    # 创建临时投影层
    img_proj = nn.Linear(img_features.shape[1], img_dim).cuda()
    rel_proj = nn.Linear(rel_features.shape[1], rel_dim).cuda()
    # 投影特征到相同维度
    img_features_proj = img_proj(torch.tensor(img_features, dtype=torch.float32).cuda())
    rel_features_proj = rel_proj(torch.tensor(rel_features, dtype=torch.float32).cuda())
    img_features_proj = F.normalize(img_features_proj, dim=1).cpu().detach().numpy()
    rel_features_proj = F.normalize(rel_features_proj, dim=1).cpu().detach().numpy()
    # 计算模态一致性（使用投影后的特征）
    modal_consistency = np.zeros(ENT_NUM, dtype=np.float32)
    for i in range(ENT_NUM):
        # 文本特征（如果有）
        if name_features is not None:
            sim_tv = cosine_similarity(name_features[i], img_features_proj[i])
            sim_ts = cosine_similarity(name_features[i], rel_features_proj[i])
        else:
            sim_tv = sim_ts = 0
            
        sim_vs = cosine_similarity(img_features_proj[i], rel_features_proj[i])
    ...
    # 修改train_ill的创建方式
    train_ill = EADataset(
        np.array(ills[:int(len(ills) // 1 * args.data_rate)], dtype=np.int32),
        difficulty_dict=sample_difficulty
    )
