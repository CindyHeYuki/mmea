...
class Runner:
    ...
    def alpha_sweep(self, test_left, test_right):
        ...
        for c_alpha in causal_alphas:
            for s_alpha in csc_alphas:
                for n_alpha in neighbor_alphas:
                    counter += 1
                    self.logger.info(f"\n--- [{counter}/{total}] causal_α={c_alpha}, csc_α={s_alpha}, neighbor_α={n_alpha} ---")
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"{'排名':<6}{'causal_α':<12}{'csc_α':<10}{'neighbor_α':<12}{'Hits@1':<10}{'Hits@10':<10}{'MRR':<10}")
        self.logger.info("-" * 90)
        for i, r in enumerate(results_sorted[:15]):
            marker = "⭐" if i == 0 else "  "
            self.logger.info(f"{marker} {i+1:<4}{r['causal_alpha']:<12}{r['csc_alpha']:<10}{r['neighbor_alpha']:<12}"
